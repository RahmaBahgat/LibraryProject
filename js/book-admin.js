books = [
  ...
  },
  {
    ...
    author: "Rebecca Yarros",
    cover:
      "../images/books/Onyx Storm von Rebecca Yarros - Taschenbuch - 978-0-349-43707-1 _ Thalia.jpeg",
    genre: "Fiction",
...
function validation() {
  ...
  var category1 = document.getElementById("category 1").value.toLowerCase();
  var category2 = document.getElementById("category 2").value.toLowerCase();
  var category3 = document.getElementById("category 3").value.toLowerCase();
  var image = document.getElementById("image").value.trim();
  var errorBox = document.getElementById("error");

  if (name.length < 3) {
    errorBox.innerHTML = "Name should be more than 3 characters";
    return false;
  }

  if (image.length < 6 || !image.startsWith("http")) {
    errorBox.innerHTML = "enter valid url";
    return false;
  }

  if (author.length < 3) {
    errorBox.innerHTML = "Author should be more than 3 characters";
    return false;
  }

  if (category1 == 0 && category2.length == 0 && category3.length == 0) {
    errorBox.innerHTML = "Please select at least one category";
    return false;
  }
...
function clearInputs() {
  ...
  document.getElementById("category 1").value = "";
...
function addBook() {
  ...
  var category1 = document.getElementById("category 1").value;
  var category2 = document.getElementById("category 2").value;
  var category3 = document.getElementById("category 3").value;
  var image = document.getElementById("image").value;
  var description = document.getElementById("description").value;

  if (validation()) {
    let maxId = books.reduce((max, book) => Math.max(max, Number(book.id)), 0);
    books.push({
      id: maxId + 1,
      title: name,
      author: author,
...
function deleteBook() {
  ...
  // Find the index of the book based on the name
